...
class TestCommands:
    ...
    async def test_show_command(self, mock_interaction, test_characters_collection):
        ...
        with patch("utils.characters_collection", test_characters_collection):
            ...
            async def mock_show_character(interaction, character_name):
                characters = utils.get_all_user_characters_for_user(
                    str(interaction.user.id)
                )
                char = next(
                    (c for c in characters if c.character == character_name), None
                )
                if char:
            ...
            assert (
                character_name in mock_interaction.response.send_message.call_args[0][0]
            )
    ...
    async def test_plus_command(self, mock_interaction, test_characters_collection):
        ...
        with (
            ...
            patch(
                ...
                return_value=str(valid_object_id),
            ),
        ):
            user_id = str(mock_interaction.user.id)
            character_name = "Test Character"
            ...
            def mock_get_counters(char_id):
                return [
                    {
                        "counter": counter_name,
                        "temp": counter_state["temp"],
                        "perm": counter_state["perm"],
                        "category": "general",
                        "comment": "",
                        "counter_type": counter_type,
                    }
            ...
            with patch("utils.get_counters_for_character", mock_get_counters):
                ...
                async def mock_plus_cmd(
                    interaction, character_name, counter_name, amount
                ):
                    ...
                    counter = next(
                        (c for c in counters if c["counter"] == counter_name), None
                    )
                    if not counter:
                        await interaction.response.send_message(
                            f"Counter not found: {counter_name}"
                        )
                        return
                    new_temp = counter["temp"] + amount
                    if amount < 0 and new_temp < 0:
                        await interaction.response.send_message(
                            "Cannot decrement temp below zero"
                        )
                        return
                    elif (
                        amount > 0
                        and counter.get("counter_type") == "perm_is_maximum"
                        and new_temp > counter["perm"]
                    ):
                        counter_state["temp"] = counter["perm"]
                    else:
                        counter_state["temp"] = new_temp
                    await interaction.response.send_message(
                        f"Updated {counter_name} by {amount}"
                    )

                # Decrement below zero
                await mock_plus_cmd(mock_interaction, character_name, counter_name, -20)
                mock_interaction.response.send_message.assert_any_call(
                    "Cannot decrement temp below zero"
                )
    ...
    @pytest.mark.asyncio
    async def test_add_character_with_sanitization(
        self, mock_interaction, test_characters_collection
    ):
        # Patch the characters_collection with our test collection
        with patch("utils.characters_collection", test_characters_collection):
            ...
            async def mock_add_character(interaction, character_name):
                ...
                if success:
                    await interaction.response.send_message(
                        f"Added character: {character_name}"
                    )
                else:
    ...
    @pytest.mark.asyncio
    async def test_add_duplicate_character(
        self, mock_interaction, test_characters_collection
    ):
        # Patch the characters_collection with our test collection
        with patch("utils.characters_collection", test_characters_collection):
    ...
    async def test_add_counter_with_sanitization(self, test_characters_collection):
        ...
        with patch("utils.characters_collection", test_characters_collection):
            ...
            counter = next(
                (c for c in counters if getattr(c, "counter", None) == "CounterName"),
                None,
            )
            assert counter is not None
