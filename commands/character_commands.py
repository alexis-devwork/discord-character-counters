...
def register_character_commands(cog):
    ...
    async def _validate_new_bedlam_value(target, new_value, interaction):
        ...
        if new_value < 0:
            await interaction.response.send_message(
                "Bedlam cannot be negative.", ephemeral=True
            )
            return None, False
        if new_value > target.perm:
            await interaction.response.send_message(
                f"Bedlam cannot exceed perm ({target.perm}).", ephemeral=True
            )
            return None, False
        return new_value, True

    def _update_counter_in_mongodb(
        character_id, counter_name, field, value, target=None
    ):
    ...
    async def _send_counter_response(interaction, character, msg, public=False):
        await interaction.response.send_message(
            f"Counters for character '{character}':\n{msg}", ephemeral=not public
        )

    async def _handle_counter_update(
        interaction, character, counter, target, field, value, update_func, public=False
    ):
        # Validate new value
        if field == "temp":
            adjusted_value, is_valid = await _validate_new_temp_value(
    ...
    @cog.avct_group.command(
        name="show", description="Show all counters and health for a character"
    )
    ...
    async def list_characters(interaction: discord.Interaction):
        ...
        if not entries:
            await interaction.response.send_message(
                "No characters found.", ephemeral=True
            )
            return
        msg = "\n".join(
            [
                f"ID: {getattr(e, 'id', 'N/A')}, Character: {e.character}"
                for e in entries
            ]
        )
        await interaction.response.send_message(
            f"Characters for you:\n{msg}", ephemeral=True
        )

    @cog.character_group.command(
        name="temp", description="Set temp value for a counter"
    )
    ...
    async def temp(
        ...
        await _handle_counter_update(
            ...
            lambda: _update_counter_in_mongodb(
                character_id, counter, "temp", target.temp
            ),
        )

    @cog.character_group.command(
        name="perm", description="Set perm value for a counter"
    )
    @discord.app_commands.autocomplete(
        character=character_name_autocomplete,
        counter=counter_name_autocomplete_for_character,
    ...
    @discord.app_commands.autocomplete(character=character_name_autocomplete)
    async def reset_eligible_cmd(interaction: discord.Interaction, character: str):
        user_id = str(interaction.user.id)
        character_id = get_character_id_by_user_and_name(user_id, character)
        if character_id is None:
            await handle_character_not_found(interaction)
