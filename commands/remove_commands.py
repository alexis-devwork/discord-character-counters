...
def register_remove_commands(cog):
    ...
    async def remove_character_cmd(
        ...
        if success:
            ...
            await handle_character_not_found(interaction) if error == "Character not found." else interaction.response.send_message(error or "Failed to remove character.", ephemeral=True)
